...
Template for generating Jest test scaffolds from TypeScript source files.
...
jest.mock('{dependency_path}', () => ({
  {dependency_name}: jest.fn(),
}));

// In test:
const mock{Dependency} = {dependency_name} as jest.MockedFunction<typeof {dependency_name}>;
...
| Feature | Jest | Vitest |
|---------|------|--------|
| Import | No import needed (globals) | `import { describe, it, expect } from 'vitest'` |
| Mock | `jest.mock()`, `jest.fn()` | `vi.mock()`, `vi.fn()` |
| Clear mocks | `jest.clearAllMocks()` | `vi.clearAllMocks()` |
| Fail assertion | `throw new Error()` | `expect.fail()` |
